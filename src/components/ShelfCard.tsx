...
const ShelfCard = ({ segment, segmentedVideoId, isPlaceholder }: ShelfCardProps) => {
    if (isPlaceholder) {
        return (
            <Link
                ...
                className="min-w-[120px] w-32 md:min-w-[160px] md:w-48 aspect-[3/4] flex flex-col items-center justify-center border-2 border-dashed border-border rounded-2xl no-underline text-muted-foreground gap-2 snap-start hover:border-accent hover:text-accent transition-all shadow-sm bg-muted/20"
            >
    ...
    return (
        <Link
            ...
            className="min-w-[140px] w-36 md:min-w-[192px] md:w-48 flex flex-col no-underline text-inherit snap-start group"
        >
            <SegmentThumbnail
